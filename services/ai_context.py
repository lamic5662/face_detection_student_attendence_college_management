...
_INTENTS = {
    'attendance': [
        'absent', 'attendance', 'present', 'miss', 'low attendance',
        'who absent', 'attendance record', 'bunk', 'attandance',
    ],
    'marks': [
        'mark', 'grade', 'score', 'exam', 'result', 'fail', 'pass',
        'percentage', 'topper', 'rank', 'gpa', 'merit', 'highest', 'lowest',
    ],
    'fee': [
        'fee', 'payment', 'paid', 'due', 'pending', 'outstanding',
        'receipt', 'balance', 'unpaid',
    ],
    'notes': [
        'note', 'notes', 'material', 'content', 'lesson', 'chapter',
        'topic', 'summarize', 'summary', 'explain', 'study', 'resource',
    ],
    'assignment': [
        'assignment', 'homework', 'submit', 'submission', 'task',
        'pending assignment', 'due', 'lab',
    ],
    'students': [
        'student', 'students', 'enrolled', 'roll number', 'classmate',
        'how many student', 'list of student',
    ],
    'teachers': [
        'teacher', 'faculty', 'instructor', 'lecturer', 'professor',
        'how many teacher',
    ],
    'notice': [
        'notice', 'announcement', 'notification', 'circular', 'bulletin',
    ],
    'timetable': [
        'timetable', 'schedule', 'class time', 'period', 'when is class',
        'which class', 'today class',
    ],
    'leave': [
        'leave', 'sick leave', 'leave request', 'leave application',
        'approved leave',
    ],
}
...
def _ctx_admin(user: 'User', college_id: int, intents: set[str], query: str) -> str:
    ...
    if 'students' in intents or 'attendance' in intents:
        ...
        for s in students[:50]:
            ...
            lines.append(f"  - {s.user.full_name} | Roll:{s.roll_number} | Dept:{dept} | Sem:{s.semester}")
    ...
    if 'teachers' in intents:
        ...
        for t in teachers:
            ...
            lines.append(f"  - {t.user.full_name} | {t.designation or 'Teacher'} | Dept:{dept}")
    ...
    if 'attendance' in intents:
        ...
        parts.append("MOST ABSENT STUDENTS (top 15):\n" + ('\n'.join(lines) or '  No records yet.'))
        ...
        parts.append("SUBJECT-WISE TOTAL ABSENCES:\n" + ('\n'.join(subj_lines) or '  No records yet.'))
    ...
    if 'marks' in intents:
        ...
        for m in marks:
            if m.marks_obtained is not None and not m.is_absent:
                ...
                if ex and ex.total_marks:
                    stu_scores[m.student_id].append(m.marks_obtained / ex.total_marks * 100)

        lines = []
        for stu_id, scores in sorted(stu_scores.items(), key=lambda x: -sum(x[1]) / len(x[1]))[:10]:
            s = stu_map.get(stu_id)
            if not s:
                continue
            avg = sum(scores) / len(scores)
            lines.append(f"  - {s.user.full_name} | Roll:{s.roll_number} | Avg:{avg:.1f}%")
        parts.append("TOP PERFORMERS (by average %):\n" + ('\n'.join(lines) or '  No marks yet.'))
        ...
        for stu_id, scores in sorted(stu_scores.items(), key=lambda x: sum(x[1]) / len(x[1]))[:10]:
            ...
            if not s:
                continue
            avg = sum(scores) / len(scores)
            bottom.append(f"  - {s.user.full_name} | Roll:{s.roll_number} | Avg:{avg:.1f}%")
        parts.append("LOWEST PERFORMERS:\n" + ('\n'.join(bottom) or '  No marks yet.'))

    if 'fee' in intents:
        structures = FeeStructure.query.filter_by(college_id=college_id, is_active=True).all()
        ...
        for fs in structures:
            for s in students:
                if fs.id not in paid_set[s.id]:
                    unpaid.append(f"  - {s.user.full_name} | Roll:{s.roll_number} | Unpaid: {fs.title}")
        parts.append("UNPAID FEES (first 20):\n" + ('\n'.join(unpaid[:20]) or '  All paid.'))

    if 'notice' in intents:
        from models.notice import Notice
        notices = Notice.query.filter_by(college_id=college_id).order_by(Notice.created_at.desc()).limit(10).all()
        lines = [f"  - [{n.category.upper()}] {n.title} (posted {n.created_at.strftime('%Y-%m-%d')})" for n in notices]
        parts.append("RECENT NOTICES:\n" + ('\n'.join(lines) or '  No notices.'))

    if 'leave' in intents:
        from models.leave import LeaveRequest
        leaves = LeaveRequest.query.filter_by(college_id=college_id).order_by(LeaveRequest.created_at.desc()).limit(20).all()
        stu_map = {s.id: s for s in students}
        lines = []
        for lv in leaves:
            s = stu_map.get(lv.student_id)
            name = s.user.full_name if s else 'Unknown'
            lines.append(f"  - {name} | {lv.from_date} to {lv.to_date} | Status:{lv.status} | Reason:{lv.reason[:60]}")
        parts.append("LEAVE REQUESTS (recent 20):\n" + ('\n'.join(lines) or '  None.'))

    if 'assignment' in intents:
        from models.content import TeacherContent
        from models.assignment import AssignmentSubmission
        assignments = TeacherContent.query.filter_by(
            college_id=college_id, content_type='assignment', is_published=True
        ).order_by(TeacherContent.created_at.desc()).limit(10).all()
        lines = []
        for a in assignments:
            subs = AssignmentSubmission.query.filter_by(content_id=a.id).count()
            lines.append(f"  - {a.title} | Due:{a.due_date or 'N/A'} | Submissions:{subs}")
...
def _ctx_teacher(user: 'User', college_id: int, intents: set[str], query: str) -> str:
    ...
    parts = [f"YOUR PROFILE: {user.full_name} | {teacher.designation or 'Teacher'} | Employee ID: {teacher.employee_id}"]

    subjects = Subject.query.filter_by(teacher_id=teacher.id, college_id=college_id).all()
    subj_lines = [f"  - {s.name} ({s.code}) | Sem:{s.semester}" for s in subjects]
    parts.append("YOUR SUBJECTS:\n" + ('\n'.join(subj_lines) or '  None assigned.'))

    if 'attendance' in intents:
        sessions = AttendanceSession.query.filter_by(teacher_id=teacher.id, college_id=college_id).all()
        subj_map = {s.id: s.name for s in subjects}
        records = []
        ...
        parts.append(f"MOST ABSENT IN YOUR CLASSES:\n" + ('\n'.join(lines) or '  No records yet.'))

    if 'notes' in intents or 'assignment' in intents:
        contents = TeacherContent.query.filter_by(
            teacher_id=teacher.id, college_id=college_id, is_published=True
        ).order_by(TeacherContent.created_at.desc()).limit(15).all()
        ...
        if notes and 'notes' in intents:
            note_q = query.lower()
            matched = [n for n in notes if any(w in (n.title + ' ' + (n.body or '')).lower() for w in note_q.split() if len(w) > 3)]
        ...
        if asgns and 'assignment' in intents:
            ...
            for a in asgns[:5]:
                subs = AssignmentSubmission.query.filter_by(content_id=a.id).count()
                a_lines.append(f"  - {a.title} | Due:{a.due_date or 'N/A'} | Submissions:{subs} | Marks:{a.marks}")
            parts.append("YOUR ASSIGNMENTS:\n" + '\n'.join(a_lines))

    if 'marks' in intents:
        exams = Exam.query.filter_by(college_id=college_id, created_by=teacher.id).all()
        exam_map = {e.id: e for e in exams}
        marks = Mark.query.filter(Mark.exam_id.in_([e.id for e in exams])).all() if exams else []
        stu_scores: dict[int, list] = defaultdict(list)
        for m in marks:
            if m.marks_obtained is not None:
                ex = exam_map.get(m.exam_id)
                if ex:
                    stu_scores[m.student_id].append((ex.title, m.marks_obtained, ex.total_marks))
        students = Student.query.filter_by(college_id=college_id).all()
        stu_map = {s.id: s for s in students}
        lines = []
        ...
        parts.append("MARKS ENTERED BY YOU:\n" + ('\n'.join(lines) or '  No marks entered.'))
...
def _ctx_student(user: 'User', college_id: int, intents: set[str], query: str) -> str:
    ...
    if 'attendance' in intents:
        ...
        sessions = AttendanceSession.query.filter_by(college_id=college_id).all()
        sess_subj = {s.id: s.subject_id for s in sessions}
        subjects = Subject.query.filter_by(college_id=college_id, department_id=student.department_id).all()
        subj_map = {s.id: s.name for s in subjects}
        ...
        lines = [f"  Overall: {overall_present}/{len(records)} | {_pct(overall_present, len(records))}"]
        for sid, tot in sorted(subj_total.items()):
            p = subj_present[sid]
            lines.append(f"  - {subj_map.get(sid, 'Unknown')}: {p}/{tot} ({_pct(p, tot)})")
    ...
    if 'marks' in intents:
        ...
        subjects = {s.id: s.name for s in Subject.query.filter_by(college_id=college_id).all()}
        lines = []
        for m in marks:
            ...
            if ex:
                ...
                if m.is_absent:
                    ...
                    lines.append(f"  - {ex.title} ({subj}): {m.marks_obtained}/{ex.total_marks} ({pct:.1f}%)")
        parts.append("YOUR MARKS:\n" + ('\n'.join(lines) or '  No marks recorded yet.'))

    if 'fee' in intents:
        structures = FeeStructure.query.filter_by(college_id=college_id, is_active=True).all()
        payments = FeePayment.query.filter_by(student_id=student.id, college_id=college_id).all()
        paid_ids = {p.fee_structure_id for p in payments}
        lines = []
        for fs in structures:
            status = 'PAID' if fs.id in paid_ids else 'UNPAID'
            lines.append(f"  - {fs.title}: Rs.{fs.amount} | {status} | Due:{fs.due_date or 'N/A'}")
        parts.append("YOUR FEES:\n" + ('\n'.join(lines) or '  No fee structures set.'))

    if 'notes' in intents:
        notes = TeacherContent.query.filter_by(
            college_id=college_id,
            department_id=student.department_id,
            semester=student.semester,
            is_published=True,
        ).filter(TeacherContent.content_type.in_(['note', 'lab'])).order_by(
            TeacherContent.created_at.desc()
        ).limit(50).all()
        ...
        has_specific_topic = best_score > 0 and best_note and (best_note.body or '').strip()

        if has_specific_topic:
            body = best_note.body.strip()
        ...
        elif not q_words or all(w in ('note', 'notes', 'summarize', 'summary', 'show', 'list') for w in q_words):
    ...
    if 'timetable' in intents:
        ...
        lines = []
        for entry in tt:
            day = days[entry.day_of_week] if 0 <= entry.day_of_week < len(days) else str(entry.day_of_week)
            lines.append(f"  {day}: {entry.start_time.strftime('%H:%M')}-{entry.end_time.strftime('%H:%M')} | {subjects.get(entry.subject_id, 'Unknown')}")
        parts.append("YOUR TIMETABLE:\n" + ('\n'.join(lines) or '  No timetable set.'))

    if 'leave' in intents:
        leaves = LeaveRequest.query.filter_by(student_id=student.id, college_id=college_id).order_by(
...
def _ctx_parent(user: 'User', college_id: int, intents: set[str], query: str) -> str:
    ...
    parent = Parent.query.filter_by(user_id=user.id, college_id=college_id).first()
    if not parent:
        return "Parent profile not found."

    children = Student.query.filter_by(college_id=college_id).filter(
        Student.id.in_([c.id for c in (parent.students or [])])
    ).all()
    ...
    for child in children:
        ...
        if 'attendance' in intents:
            records = AttendanceRecord.query.filter_by(student_id=child.id, college_id=college_id).all()
            present = sum(1 for r in records if r.status == 'present')
            child_parts.append(f"Attendance: {present}/{len(records)} sessions ({_pct(present, len(records))})")

        if 'marks' in intents:
            marks = Mark.query.filter_by(student_id=child.id, college_id=college_id).all()
            exams = {e.id: e for e in Exam.query.filter_by(college_id=college_id).all()}
            subjects = {s.id: s.name for s in Subject.query.filter_by(college_id=college_id).all()}
            for m in marks[:10]:
                ex = exams.get(m.exam_id)
        ...
        if 'fee' in intents:
            structs = FeeStructure.query.filter_by(college_id=college_id, is_active=True).all()
            paid_ids = {p.fee_structure_id for p in FeePayment.query.filter_by(student_id=child.id).all()}
            for fs in structs:
                status = 'PAID' if fs.id in paid_ids else 'UNPAID'
    ...
    if 'notice' in intents:
        notices = Notice.query.filter_by(college_id=college_id).order_by(
