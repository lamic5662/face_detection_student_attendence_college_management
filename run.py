...
from models.parent import ClassAlert, ParentStudent, TeacherStatus  # noqa: F401
...
def _port_accepting_connections(
...
def _clear_cluster_pid_state(port: int) -> None:
    path = _cluster_pid_path(port)
    if path.exists():
        path.unlink()


def _cluster_instance_ready(port: int, pid: int | None = None) -> bool:
    if pid is None:
        pid = _stored_cluster_pid(port)
    if not pid or not _is_pid_running(pid):
        return False
    return _port_accepting_connections(port)


def _wait_for_cluster_instance(
    port: int, pid: int | None = None, *, timeout: float = 8.0, interval: float = 0.25
) -> bool:
    deadline = time.time() + timeout
    while time.time() < deadline:
        if _cluster_instance_ready(port, pid):
            return True
        time.sleep(interval)
    return _cluster_instance_ready(port, pid)
...
def _start_cluster_instance(port: int) -> tuple[int, Path]:
    ...
    if not gunicorn:
        raise RuntimeError(
            "gunicorn is not installed in the current Python environment."
        )
...
def create_admin():
    ...
    with app.app_context():
        ...
        if User.query.filter_by(college_id=college.id, email=email).first():
            ...
            return
        u = User(
            college_id=college.id, name=name, email=email, role="admin", is_active=True
        )
        u.set_password(pw)
        db.session.add(u)
        db.session.commit()
...
def create_super_admin():
    ...
    with app.app_context():
        ...
        name = input("Full name: ").strip()
        pw = input("Password: ").strip()
        if User.query.filter_by(email=email, role="super_admin").first():
            print(f"Super admin {email} already exists.")
            return
        u = User(
            college_id=None, name=name, email=email, role="super_admin", is_active=True
        )
        u.set_password(pw)
        db.session.add(u)
        db.session.commit()
    ...
    with app.app_context():
        ...
        for slot in slots:
            ...
            students = Student.query.filter_by(
                department_id=slot.department_id, semester=slot.semester
            ).all()
            ...
            total_sent += sent
            print(
                f"  Alert sent: {slot.subject.name} @ {slot_time} — {sent} recipients"
            )
...
@app.cli.command("prepare-production")
@click.option(
    "--public-host", required=True, help="Public app host, e.g. portal.example.com"
)
@click.option(
    "--root-domain", required=True, help="Root tenant domain, e.g. example.com"
)
@click.option(
    "--app-root",
    default="/opt/smart_attendance",
    show_default=True,
    help="Deployment root path on the server.",
)
@click.option(
    "--service-user",
    default="www-data",
    show_default=True,
    help="System user that will run Gunicorn.",
)
@click.option(
    "--service-group",
    default="www-data",
    show_default=True,
    help="System group that will run Gunicorn.",
)
@click.option(
    "--db-name",
    default="smart_attendance",
    show_default=True,
...
@click.option(
    ...
    show_default=True,
    help="Where to write the generated deployment bundle.",
)
def prepare_production(
    public_host, root_domain, app_root, service_user, service_group, db_name, output_dir
):
    """Generate deployment-ready env, Nginx, and systemd files for a real server."""
    public_host = public_host.strip().lower()
    root_domain = root_domain.strip().lower().lstrip(".")
    app_root = app_root.rstrip("/")

    if not public_host or not root_domain:
        raise click.ClickException("public-host and root-domain are required.")
    if public_host != root_domain and not public_host.endswith(f".{root_domain}"):
        raise click.ClickException("public-host must match or be inside root-domain.")

    base_dir = Path(app.root_path)
    files = write_deployment_bundle(
    ...
    print("Production deployment bundle generated")
    print("-------------------------------------")
    print(f"Public host: {public_host}")
    print(f"Root domain: {root_domain}")
    print(f"App root: {app_root}")
    print(f"Service user/group: {service_user}:{service_group}")
    print("Files:")
    for path in files:
        print(f"  - {path}")
    print()
    print("Next step:")
    print(f"  Review {files[-1]} and then fill secrets into {(Path(files[0])).name}.")
...
def start_local_cluster():
    ...
    for port in ports:
        pid = _stored_cluster_pid(port)
        if pid and _cluster_instance_ready(port, pid):
            started.append((port, pid, _cluster_log_path(port), True))
            continue
        if pid:
            _clear_cluster_pid_state(port)

        pid, log_path = _start_cluster_instance(port)
        if not _wait_for_cluster_instance(port, pid):
            tail = log_path.read_text(encoding="utf-8", errors="ignore")[-4000:]
            _clear_cluster_pid_state(port)
            raise RuntimeError(
                f"Gunicorn instance on port {port} failed to start or did not begin accepting connections.\n"
                f"--- log: {log_path} ---\n{tail}"
            )
...
def local_cluster_status():
    ...
    for port in ports:
        ...
        ready = _cluster_instance_ready(port, pid) if pid else False
        print(
            f"  - 127.0.0.1:{port} | pid={pid or '(none)'} "
            f"| running={'yes' if running else 'no'} "
            f"| accepting={'yes' if ready else 'no'} | log={_cluster_log_path(port)}"
        )
