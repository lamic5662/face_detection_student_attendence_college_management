...
from extensions import compress, csrf, db, limiter, login_manager, mail, migrate
...
def _static_asset_version(app: Flask, relative_path: str) -> int | None:
    path = os.path.join(app.static_folder or "", relative_path)
    try:
...
def _add_health_check(app: Flask) -> None:
    ...
    def health():
        ...
        return jsonify(status=status, db=db_status, env=flask_env), (
...
def _validate_runtime_config(app: Flask) -> None:
    ...
    if resolve_library_path(app, "uploads/library/check.txt") is None:
        raise RuntimeError("LIBRARY_UPLOAD_FOLDER is invalid or unsafe.")
    ...
    if not app.debug and not app.testing:
        ...
        if (
            not app.config.get("SESSION_COOKIE_SECURE")
            and not allow_insecure_local_http
        ):
            raise RuntimeError(
                "SESSION_COOKIE_SECURE must be enabled outside development/testing."
            )
        if (
            not app.config.get("REMEMBER_COOKIE_SECURE")
            and not allow_insecure_local_http
        ):
            raise RuntimeError(
                "REMEMBER_COOKIE_SECURE must be enabled outside development/testing."
            )
        ...
        library_dir = os.path.abspath(app.config["LIBRARY_UPLOAD_FOLDER"])
        ...
        if os.path.commonpath([content_dir, static_dir]) == static_dir:
            raise RuntimeError(
                "CONTENT_UPLOAD_FOLDER must not be inside the public static directory."
            )
        if os.path.commonpath([library_dir, static_dir]) == static_dir:
            raise RuntimeError(
                "LIBRARY_UPLOAD_FOLDER must not be inside the public static directory."
            )
        if os.path.commonpath([assignment_dir, static_dir]) == static_dir:
            raise RuntimeError(
                "ASSIGNMENT_UPLOAD_FOLDER must not be inside the public static directory."
            )
    ...
    if root_domain and ("://" in root_domain or "/" in root_domain):
        raise RuntimeError(
            "MULTI_COLLEGE_ROOT_DOMAIN must be a bare domain such as example.com."
        )
...
def create_app(config_override=None) -> Flask:
    ...
    def _exempt_safe_reads():
        # Keep explicit POST rate limits for login/password flows, but do not
        # rate-limit normal page navigation and polling reads.
    ...
    def _csrf_exempt_ajax():
        ...
        if (
            ...
            in {"student", "teacher", "parent", "sub_admin", "librarian"}
    ...
    def inject_globals():
        ...
        try:
            ...
            college = None if is_platform_scope else get_current_college(optional=True)
            from models.setting import CollegeSetting

            if (
                current_user.is_authenticated
                and not is_platform_scope
                and getattr(current_user, "college", None) is not None
            ):
            ...
            brand_name = (
                ...
                else app.config.get("COLLEGE_NAME", "College")
            )
            notification_items = []
            notification_count = 0
            sidebar_navigation = None
            notices_enabled = False
            notices_feature_enabled = False
            notification_mode = ""
            notification_board_url = ""
            ai_assistant_enabled = False
            ai_assistant_unavailable_reason = ""
            ...
            if current_user.is_authenticated:
                if current_user.role == "super_admin":
                    ...
                    try:
                        from services.ai_service import unavailable_reason

                        ai_assistant_unavailable_reason = unavailable_reason() or ""
                        ai_assistant_enabled = not ai_assistant_unavailable_reason
                    except Exception:
                        ai_assistant_enabled = False
                        ai_assistant_unavailable_reason = (
                    ...
                    notices_feature_enabled = user_has_feature(current_user, "notices")
                    library_feature_enabled = user_has_feature(current_user, "library")
                    notices_enabled = notices_feature_enabled or library_feature_enabled
                    ai_feature_enabled = user_has_feature(current_user, "ai_assistant")
                    if ai_feature_enabled:
                        ...
                        ai_assistant_unavailable_reason = (
                            "AI Assistant is disabled for your college."
                        )
                if notices_enabled and current_user.role != "super_admin":
                    notification_payload = notification_center_payload(
                        current_user,
                        include_public_notices=notices_feature_enabled,
                    )
                    notification_items = notification_payload["items"]
                    notification_count = notification_payload["count"]
                    notification_mode = "notice"
                    notification_board_url = (
                        url_for("notice.list_notices")
                        if notices_feature_enabled
                        else private_notification_board_url(current_user)
                    )
                sidebar_navigation = build_sidebar_navigation(
                    current_user, request.endpoint
                )

            return dict(
                college_name=cs.college_name if cs is not None else brand_name,
                college_lat=(
                    cs.latitude
                    if cs is not None and cs.latitude is not None
                    else app.config["COLLEGE_LAT"]
                ),
                college_lng=(
                    cs.longitude
                    if cs is not None and cs.longitude is not None
                    else app.config["COLLEGE_LNG"]
                ),
                college_logo=cs.logo_path if cs is not None else None,
                college_logo_version=(
                    int(cs.updated_at.timestamp())
                    if cs is not None and cs.logo_path and cs.updated_at
                    else None
                ),
                style_css_version=_static_asset_version(
                    app, os.path.join("css", "style.css")
                ),
                now=_dt.now,
                ...
                notification_board_url=notification_board_url,
                ai_assistant_enabled=ai_assistant_enabled,
                ai_assistant_unavailable_reason=ai_assistant_unavailable_reason,
                sidebar_navigation=sidebar_navigation,
                current_college=None if is_platform_scope else college,
            )
        except Exception:
            return dict(
...
def _start_scheduler(app: "Flask") -> None:
    ...
    try:
        ...
        logging.getLogger(__name__).info(
            "APScheduler started — hourly background jobs (attendance reports + fee reminders)"
        )
